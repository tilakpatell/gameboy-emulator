	...
	if (ie_bit & if_bit & 0x1F) {
		...
		for (int i = 0; i < 5; i++) {
			if ((if_bit & ie_bit) & (1 << i)) {
				...
				mmu.write(0xFF0F, if_bit & ~(1 << i));   // clear the flag
				push_stack(regs.pc);                       // save where we were
				regs.pc = 0x0040 + (i * 0x08);            // jump to handler
...
void CPU::step() {
	...
	switch (opcode) {
		...
		// 0x00 - 0x0F
	case 0x00: break;
	case 0x01: regs.bc = fetch_word(); break;
	case 0x02: mmu.write(regs.bc, regs.get_a()); break;
	...
	case 0x06: regs.set_b(fetch_byte()); break;
	case 0x07: {
		u8 old_a = regs.get_a();
		u8 bit7 = (old_a >> 7) & 1;
	...
	case 0x0A: regs.set_a(mmu.read(regs.bc)); break;
	...
	case 0x0E: regs.set_c(fetch_byte()); break;
	case 0x0F: {
		u8 old_a = regs.get_a();
		u8 bit0 = old_a & 1;
		...
		break;
			 ...
			 // 0x10 - 0x1F
	case 0x10: halted = true; break;
	case 0x11: regs.de = fetch_word(); break;
	case 0x12: mmu.write(regs.de, regs.get_a()); break;
	...
	case 0x16: regs.set_d(fetch_byte()); break;
	case 0x17: {
		u8 old_a = regs.get_a();
	...
	case 0x1A: regs.set_a(mmu.read(regs.de)); break;
	...
	case 0x1E: regs.set_e(fetch_byte()); break;
	case 0x1F: {
		u8 old_a = regs.get_a();
		u8 old_carry = get_flag(Flag::Carry) ? 1 : 0;
	...
	case 0x20: {
		i8 offset = static_cast<i8>(fetch_byte());
		if (!get_flag(Flag::Zero)) { regs.pc += offset; }
		break;
	}
	case 0x21: regs.hl = fetch_word(); break;
	case 0x22: mmu.write(regs.hl, regs.get_a()); regs.hl++; break;
	...
	case 0x26: regs.set_h(fetch_byte()); break;
	case 0x27: {
		u8 a = regs.get_a();
	...
	case 0x2A: regs.set_a(mmu.read(regs.hl)); regs.hl++; break;
	...
	case 0x2E: regs.set_l(fetch_byte()); break;
	case 0x2F:
	...
	case 0x30: {
		i8 offset = static_cast<i8>(fetch_byte());
		if (!get_flag(Flag::Carry)) { regs.pc += offset; }
		break;
	}
	case 0x31: regs.sp = fetch_word(); break;
	case 0x32: mmu.write(regs.hl, regs.get_a()); regs.hl--; break;
	...
	case 0x36: mmu.write(regs.hl, fetch_byte()); break;
	case 0x37:
		set_flag(Flag::Subtract, false);
		set_flag(Flag::HalfCarry, false);
	...
	case 0x3A: regs.set_a(mmu.read(regs.hl)); regs.hl--; break;
	...
	case 0x3E: regs.set_a(fetch_byte()); break;
	case 0x3F:
		set_flag(Flag::Subtract, false);
		set_flag(Flag::HalfCarry, false);
	...
	case 0xBF: set_flag(Flag::Zero, true); set_flag(Flag::Subtract, true); set_flag(Flag::HalfCarry, false); set_flag(Flag::Carry, false); break;
		...
		// 0xC0 - 0xFF
	case 0xC0: if (!get_flag(Flag::Zero)) { regs.pc = pop_stack(); } break;
	case 0xC1: regs.bc = pop_stack(); break;
	case 0xC2: { u16 addr = fetch_word(); if (!get_flag(Flag::Zero)) { regs.pc = addr; } break; }
	case 0xC3: regs.pc = fetch_word(); break;
	case 0xC4: { u16 addr = fetch_word(); if (!get_flag(Flag::Zero)) { push_stack(regs.pc); regs.pc = addr; } break; }
	case 0xC5: push_stack(regs.bc); break;
	case 0xC6: { u8 a = regs.get_a(); u8 v = fetch_byte(); u8 r = a + v; regs.set_a(r); set_flag(Flag::Zero, r == 0); set_flag(Flag::Subtract, false); set_flag(Flag::HalfCarry, ((a & 0x0F) + (v & 0x0F)) > 0x0F); set_flag(Flag::Carry, (static_cast<u16>(a) + v) > 0xFF); break; }
	case 0xC7: push_stack(regs.pc); regs.pc = 0x0000; break;
	case 0xC8: if (get_flag(Flag::Zero)) { regs.pc = pop_stack(); } break;
	case 0xC9: regs.pc = pop_stack(); break;
	case 0xCA: { u16 addr = fetch_word(); if (get_flag(Flag::Zero)) { regs.pc = addr; } break; }
	...
	case 0xCC: { u16 addr = fetch_word(); if (get_flag(Flag::Zero)) { push_stack(regs.pc); regs.pc = addr; } break; }
	case 0xCD: { u16 addr = fetch_word(); push_stack(regs.pc); regs.pc = addr; break; }
	case 0xCE: { u8 a = regs.get_a(); u8 v = fetch_byte(); u8 c = get_flag(Flag::Carry) ? 1 : 0; u8 r = a + v + c; regs.set_a(r); set_flag(Flag::Zero, r == 0); set_flag(Flag::Subtract, false); set_flag(Flag::HalfCarry, ((a & 0x0F) + (v & 0x0F) + c) > 0x0F); set_flag(Flag::Carry, (static_cast<u16>(a) + v + c) > 0xFF); break; }
	case 0xCF: push_stack(regs.pc); regs.pc = 0x0008; break;
	case 0xD0: if (!get_flag(Flag::Carry)) { regs.pc = pop_stack(); } break;
	case 0xD1: regs.de = pop_stack(); break;
	case 0xD2: { u16 addr = fetch_word(); if (!get_flag(Flag::Carry)) { regs.pc = addr; } break; }
	case 0xD4: { u16 addr = fetch_word(); if (!get_flag(Flag::Carry)) { push_stack(regs.pc); regs.pc = addr; } break; }
	case 0xD5: push_stack(regs.de); break;
	case 0xD6: { u8 a = regs.get_a(); u8 v = fetch_byte(); regs.set_a(a - v); set_flag(Flag::Zero, (u8)(a - v) == 0); set_flag(Flag::Subtract, true); set_flag(Flag::HalfCarry, (a & 0x0F) < (v & 0x0F)); set_flag(Flag::Carry, a < v); break; }
	case 0xD7: push_stack(regs.pc); regs.pc = 0x0010; break;
	case 0xD8: if (get_flag(Flag::Carry)) { regs.pc = pop_stack(); } break;
	case 0xD9: regs.pc = pop_stack(); ime = true; break;
	case 0xDA: { u16 addr = fetch_word(); if (get_flag(Flag::Carry)) { regs.pc = addr; } break; }
	case 0xDC: { u16 addr = fetch_word(); if (get_flag(Flag::Carry)) { push_stack(regs.pc); regs.pc = addr; } break; }
	case 0xDE: { u8 a = regs.get_a(); u8 v = fetch_byte(); u8 c = get_flag(Flag::Carry) ? 1 : 0; u8 r = a - v - c; regs.set_a(r); set_flag(Flag::Zero, r == 0); set_flag(Flag::Subtract, true); set_flag(Flag::HalfCarry, (a & 0x0F) < (v & 0x0F) + c); set_flag(Flag::Carry, static_cast<u16>(a) < static_cast<u16>(v) + c); break; }
	case 0xDF: push_stack(regs.pc); regs.pc = 0x0018; break;
	case 0xE0: mmu.write(0xFF00 + fetch_byte(), regs.get_a()); break;
	case 0xE1: regs.hl = pop_stack(); break;
	case 0xE2: mmu.write(0xFF00 + regs.get_c(), regs.get_a()); break;
	case 0xE5: push_stack(regs.hl); break;
	case 0xE6: { regs.set_a(regs.get_a() & fetch_byte()); set_flag(Flag::Zero, regs.get_a() == 0); set_flag(Flag::Subtract, false); set_flag(Flag::HalfCarry, true); set_flag(Flag::Carry, false); break; }
	case 0xE7: push_stack(regs.pc); regs.pc = 0x0020; break;
	case 0xE8: {
		i8 offset = static_cast<i8>(fetch_byte());
	...
	case 0xEA: { u16 addr = fetch_word(); mmu.write(addr, regs.get_a()); break; }
	case 0xEE: { regs.set_a(regs.get_a() ^ fetch_byte()); set_flag(Flag::Zero, regs.get_a() == 0); set_flag(Flag::Subtract, false); set_flag(Flag::HalfCarry, false); set_flag(Flag::Carry, false); break; }
	case 0xEF: push_stack(regs.pc); regs.pc = 0x0028; break;
	case 0xF0: regs.set_a(mmu.read(0xFF00 + fetch_byte())); break;
	case 0xF1: { u16 val = pop_stack(); regs.af = val & 0xFFF0; break; }
	case 0xF2: regs.set_a(mmu.read(0xFF00 + regs.get_c())); break;
	case 0xF3: ime = false; break;
	case 0xF5: push_stack(regs.af); break;
	case 0xF6: { regs.set_a(regs.get_a() | fetch_byte()); set_flag(Flag::Zero, regs.get_a() == 0); set_flag(Flag::Subtract, false); set_flag(Flag::HalfCarry, false); set_flag(Flag::Carry, false); break; }
	case 0xF7: push_stack(regs.pc); regs.pc = 0x0030; break;
	case 0xF8: {
		i8 offset = static_cast<i8>(fetch_byte());
		u8 uoffset = static_cast<u8>(offset);
		set_flag(Flag::Zero, false);
	...
	case 0xFA: { u16 addr = fetch_word(); regs.set_a(mmu.read(addr)); break; }
	case 0xFB: ime_pending = true; break;
	case 0xFE: { u8 a = regs.get_a(); u8 v = fetch_byte(); set_flag(Flag::Zero, a == v); set_flag(Flag::Subtract, true); set_flag(Flag::HalfCarry, (a & 0x0F) < (v & 0x0F)); set_flag(Flag::Carry, a < v); break; }
	case 0xFF: push_stack(regs.pc); regs.pc = 0x0038; break;
	}
}